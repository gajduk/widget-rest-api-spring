...
class MiroApplicationIntegrationTests {
    ...
    @BeforeAll
    void setUp() {
        WidgetRequest createRequest = WidgetRequest.builder().xIndex(0).yIndex(0).width(5).height(5).build();
    ...
    void createWidget_whenRequestIsInvalid_thenReturnBadRequest() throws Exception {
        WidgetRequest request = WidgetRequest.builder().build(); // Mandatory fields are not set.
    ...
    void createWidget_whenAllPropertiesGiven_thenReturnCreatedWidget() throws Exception {
        WidgetRequest request = WidgetRequest.builder().xIndex(0).yIndex(0).zIndex(3).width(5).height(5).build();
    ...
    void updateWidget_whenRequestIsInvalid_thenReturnBadRequest() throws Exception {
        ...
        mockMvc.perform(put(WIDGET_API_BY_ID_PATH, 9999)
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(request)))
                .andExpect(status().isBadRequest());
    }
    ...
    void updateWidget_whenRequestIsValid_thenReturnUpdatedWidget() throws Exception {
        ...
        WidgetRequest updateRequest = WidgetRequest.builder().xIndex(5).yIndex(0).width(5).height(5).build();
        mockMvc.perform(put(WIDGET_API_BY_ID_PATH, widget.getId())
                .contentType(MediaType.APPLICATION_JSON)
    ...
    void findAll_whenCoordinatesAreValid_thenReturnFoundWidgets() throws Exception {
        WidgetRequest createRequest1 = WidgetRequest.builder().xIndex(1000).yIndex(1000).width(100).height(100).build();
        WidgetRequest createRequest2 = WidgetRequest.builder().xIndex(1000).yIndex(1050).width(100).height(100).build();
        WidgetRequest createRequest3 = WidgetRequest.builder().xIndex(1100).yIndex(1100).width(100).height(100).build();
    ...
    private Widget saveWidget(WidgetRequest request) throws Exception {
